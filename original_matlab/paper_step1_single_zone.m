% 专门用于论文出图的脚本，由paper_plot_orginal修改而来
% 画的是162算例中的一个zone的original图
% 不必考虑函数化、规范化和封装
...
cof_marksize = 1;
active_zone = 6;
height = 5; %图窗高度。要调就在这里调整，不要去文中找参数。
width = 8.89;
...
% === 导出 ===
if info == 1
    exportgraphics(gcf,['step1_',CaseName,'withinfo','.png'],'Resolution',600); % 高分辨率导出
else
    exportgraphics(gcf,['step1_',CaseName,'.png'],'Resolution',600); % 高分辨率导出
end
