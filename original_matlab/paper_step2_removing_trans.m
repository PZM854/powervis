% 专门用于论文出图的脚本，由paper_step1_single_zone修改而来
% 画的是162算例中original图移除变压器支路后的样子。强调了zone6
% 不必考虑函数化、规范化和封装
...
% 把变压器边隐形
% 依据 zone 字段判定：zone == 0 表示该边为变压器边
if ismember('zone', G.Edges.Properties.VariableNames)
    idx_transformer = (G.Edges.zone == 0);
else
    warning('Graph edge table has no field "zone" — transformer edges not marked.');
    idx_transformer = false(size(G.Edges,1),1);
end

Color_data_now = p.EdgeColor;

% 如果当前EdgeColor是字符串（如 'black'），转为RGB矩阵
if ischar(Color_data_now) || isstring(Color_data_now)
    EdgeColorData = repmat([0 0 0], size(G.Edges,1), 1);
end

% 将zone==0的边改为白色
EdgeColorData = [];
for i = 1:size(G.Edges,1)
    if idx_transformer(i) == 1
        EdgeColorData = [EdgeColorData; [1, 1, 1]];
    else
        EdgeColorData = [EdgeColorData; [0, 0, 0]];
    end
end

% 更新绘图对象
p.EdgeColor = EdgeColorData;
...
% === 导出 ===
if info == 1
    exportgraphics(gcf,['step2_',CaseName,'withinfo','.png'],'Resolution',600); % 高分辨率导出
else
    exportgraphics(gcf,['step2_',CaseName,'.png'],'Resolution',600); % 高分辨率导出
end
