% 专门用于论文出图的脚本，由paper_plot_zonal修改而来
% 专门画进行节点聚合后、但又没进行功率整合的图。主要强调zone6
% 不必考虑函数化、规范化和封装
...
CaseName = S.CaseName;
digraph_zonescale_multi = S.digraph_zonescale_multi;

G = digraph_zonescale_multi;
%% 超参数区
...
% === 导出 ===
if info == 1
    exportgraphics(gcf,['step3_',CaseName,'withinfo','.png'],'Resolution',600); % 高分辨率导出
else
    exportgraphics(gcf,['step3_',CaseName,'.png'],'Resolution',600); % 高分辨率导出
end
